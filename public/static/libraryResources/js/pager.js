function Page(opt){
		...
		var clickpages = {
			...
			init:function(){
				...
				this.elem.children('li').click(function () {
					...
					var page = '', ele = null;
					var page1 = parseInt(clickpages.elem.children('li.active1').attr('page'));
						...
						switch (txt) {
							case '下一页':
								...
								if (page1 >= (clickpages.num - 2) || clickpages.num <= 6 || page1 < 3) {
									ele = clickpages.elem.children('li.active1').next();
								} else {
									clickpages.newPages('next', page1 + 1);
									ele = clickpages.elem.children('li.active1');
								}
								break;
							case '上一页':
								if (page1 == '1') {
								...
								if (page1 >= (clickpages.num - 1) || page1 <= 3 || clickpages.num <= 6) {
									ele = clickpages.elem.children('li.active1').prev();
								} else {
			...
			actPages:function (ele) {
				ele.addClass('active1').siblings().removeClass('active1');
				return clickpages.elem.children('li.active1').text();
			},
			JumpPages:function () {
			...
			newPages:function (type, i) {
				...
				html = '<li><a  aria-label="Previous">&laquo;</a></li>\
					<li><a>上一页</a></li>'
				for (var n = 0;n<3;n++){
					htmlC += '<li '+((n-1)==0?'class="active1"':'')+' page="'+(i+n-1)+'"><a>'+(i+n-1)+'</a></li>';
					htmlLeft += '<li '+((n+2)==i?'class="active1"':'')+' page="'+(n+2)+'"><a>'+(n+2)+'</a></li>';
					htmlRight += '<li '+((set.num+n-3)==i?'class="active1"':'')+' page="'+(set.num+n-3)+'"><a>'+(set.num+n-3)+'</a></li>';
				}
				...
				switch (type) {
					...
					case "prev":
						...
						break;
					case "«" :
						html+='<li class="active1" page="1"><a>1</a></li>'+htmlLeft+HL+'<li page="'+set.num+'"><a>'+set.num+'</a></li>';
						break;
					case "»" :
						html+='<li page="1"><a>1</a></li>'+HL+htmlRight+'<li class="active1" page="'+set.num+'"><a>'+set.num+'</a></li>';
						break;
					case "jump" :
						if(i<=4){
							if(i==1){
		...
		if(set.num<=1){
			...
			for(var i=1;i<=n;i++){
				if(i==set.startnum){
					html+='<li class="active1" page="'+i+'"><a>'+i+'</a></li>';
				}else{
